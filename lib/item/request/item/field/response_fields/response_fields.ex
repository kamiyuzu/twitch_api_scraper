defmodule TwitchApiScraper.Item.Request.Item.Field.ResponseFields do
  @moduledoc false
  ...
  defp parse_body_value({"tr", _, body_value_rows}) do
    ...
    |> Enum.with_index()
    |> Enum.reduce(%{}, fn
      {{_, _, [value]}, _}, _ when value in ["Field", "Type", "Description"] ->
        []
  ...
  defp put_value(index, value, acc) do
    case index do
      0 -> Map.put(acc, :field, String.trim(value))
